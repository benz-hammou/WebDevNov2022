...
const avarageScores = students.forEach(obj => {
    const scores = obj.scores
    const avarageOfEachStudent = scores.reduce((acc, val) => acc + val)/scores.length
    return console.log(avarageOfEachStudent); 
});

// why the result is not printing when i'm console outside ???  => console.log(avarageScores);